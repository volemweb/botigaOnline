...
<html>
    <head>
        ...
        <body>
        <?php
        if(isset($_POST['modificarPswd']))
            {
                  
                   $usuari= new Usuari($dsn, $user, $password);
               
                   $dades=$usuari->canviarPassword($_POST['pswdNou'], $_SESSION['id']);
                  
                   $usuari->closeCon();
                   
                   $_SESSION['password']=$_POST['pswdNou'];
                   
                   header('Location: pagina_usuari.php?missatge=ok');
            } 
            
        else { header('Location: pagina_usuari.php');}
        ?>
    </body>
</html>